...
function looksLikeRxLabel(fullText = "") {
  ...
  // Rx-only signals commonly present in prescription SPLs
  const rxSignals = [
    "rx only",
  ...
  // OTC/cosmetic-ish signals that should be rejected
  const otcSignals = [
    "drug facts",
  ...
  // Strict: must look Rx, and must NOT look OTC/cosmetic
  return hasRx && !hasOtc;
}
...
async function run() {
  ...
  for (let i = 0; i < drugs.length; i++) {
    ...
    console.log(`\n[${processed + 1}/${Math.min(LIMIT, drugs.length)}] Processing ${d.name} (${d.rxOnly ? 'Rx' : 'Generic'}) (RXCUI: ${d.rxcui}) ===`);
    ...
    try {
      ...
      for (const setid of setids) {
        ...
        // Validate that this looks like a real Rx label
        ...
        if (indText.length > 120 || warnText.length > 120) {
          ...
          ok++;
          enriched = true;
          console.log("  ✓ Found Rx medication label");
          break;
        } else {
