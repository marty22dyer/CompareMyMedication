...
function looksLikeRealDrugLabel(indicationsText, warningsText) {
  const t = (indicationsText + " " + warningsText).toLowerCase();

  // If it screams OTC cosmetic/topical directions only, skip for now
  const otcSignals = [
  ...
  const hits = otcSignals.filter(s => t.includes(s)).length;

  // Don't be too strict: 2+ signals = likely OTC
  return hits < 2;
}
  ...
  for (let i = 0; i < drugs.length; i++) {
    ...
    try {
      ...
      for (const setid of setids) {
        ...
        // Validate that this looks like a real drug label
        const indText = extracted.indications?.[0] || "";
        const warnText = extracted.warnings?.[0] || "";
        
        if (looksLikeRealDrugLabel(indText, warnText) && (indText.length > 120 || warnText.length > 120)) {
          d.dailymed = { setid };
          d.label = {
          ...
          ok++;
          enriched = true;
          console.log("  ✓ Found real drug label");
          break;
        } else {
          console.log(`  ✗ SETID ${setid} failed validation (OTC/insufficient content)`);
        }
      }

      if (!enriched) {
        console.log("  ↳ No valid drug label found after trying all candidates");
        fail++;
      }
